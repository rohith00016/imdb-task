...
import { fetchActors, deleteActor } from "../../redux/actions/actorActions";
...
const ActorsList = () => {
  ...
  const handleDelete = (actorId) => {
    dispatch(deleteActor(actorId, navigate));
  };
  ...
  return (
    <>
      <div className="flex justify-end w-5/6 my-6">
        <div>
          <Link to="/add-actor" className="btn btn-warning">
            Add new
          </Link>
        </div>
      </div>
        ...
        <div className="overflow-y-auto w-3/4">
          {loading ? (
            ...
            <div className="text-center mt-6">
              <p className="text-lg font-semibold">No actors found</p>
              <p>Add actors to see them listed here.</p>
            </div>
          ) : (
            <table className="table">
              <thead>
              ...
              <tbody>
                {actors.map((actor) => (
                  <tr key={actor._id}>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="avatar">
                          <div className="mask mask-squircle w-12 h-12">
                            <img src={actor.profilePic} alt="Actor" />
                          </div>
                        </div>
                        <div>
                    ...
                    <td>{actor.bio}</td>
                    <td>{actor.gender}</td>
                    <td>{formatDateDisplay(actor.dob)}</td>
                    <td>
                      <Link
                        to={`/update-actor/${actor._id}`}
